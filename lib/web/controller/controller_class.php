 ...
 * @date 2015-08-07
 */
class JController extends JControllerBase
{
	...
	public function getCaptcha()
	{
		...
		//配置参数
		$width      = intval(JReq::get('w')) == 0 ? 130 : JReq::get('w');
		$height     = intval(JReq::get('h')) == 0 ? 45  : JReq::get('h');
		$wordLength = intval(JReq::get('l')) == 0 ? 5   : JReq::get('l');
		$fontSize   = intval(JReq::get('s')) == 0 ? 25  : JReq::get('s');
	...
	public function run()
	{
		...
		JInterceptor::run("onCreateAction");
		$actionObj->run();
		JInterceptor::run("onFinishAction");
		flush();
		JF::$app->end(0);
	}
	...
	public function createAction()
	{
		...
		{
			$this->action = new JInlineAction($this,$actionId);
		}
		//2,配置动作
		...
		//3,视图动作
			...
			$this->action = new JViewAction($this,$actionId);
	...
	public function redirect($nextUrl, $location = true, $data = null)
	{
		...
		if($nextUrl[0] != '/')
		{
			...
			{
				$locationUrl = JUrl::creatUrl('/'.$this->ctrlId.'/'.$nextUrl);
			...
			{
				$this->action = new JViewAction($this,$nextUrl);
				$this->action->run();
			}
		}
		else
		{
			...
			if($location == true)
			{
				...
				$locationUrl = JUrl::creatUrl('/'.$ctrlId.'/'.$nextAction);
				header('location: '.$locationUrl);
				JF::$app->end(0);
			}
			//非重定向
			else
			{
				...
				$nextCtrlObj->init();
				$nextViewObj = new JViewAction($nextCtrlObj,$nextAction);
				$nextViewObj->run();
			}
		}
	}
