...
class JMysql extends JDB
{
	...
	public function connect($dbinfo)
	{
	  	...
	  	if(is_resource($this->linkRes))
	  	{
		  	...
		  	$DBCharset = isset(JF::$app->config['DB']['charset'])?JF::$app->config['DB']['charset'] : 'utf8';
		  	mysql_query("SET NAMES '".$DBCharset."'");
		  	if(isset(JF::$app->config['db_mode']) && JF::$app->config['db_mode'] != '')
		  	{
		  		mysql_query("SET SESSION sql_mode = '".JF::$app->config['db_mode']."' ");
		  	}
		  	else
		  	{
