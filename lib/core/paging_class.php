...
class JPaging
{
	...
	public function setSql($sql)
	{
        ...
        {
        	$count=$this->dbo->query("select count(*) as total from (".$sql.") as JPaging");
        }
	...
	public function getPageBar($url='', $attrs='')
	{
        ...
        $baseUrl = JFilter::act($baseUrl,'text');

        $attr = str_replace('[page]',1,$attrs);
        $href = str_replace("\\", "",$baseUrl.($flag?1:''));
		$tem="<ul class='pagination pages'><li><a href='{$href}' {$attr}>首页</a></li>";

        $attr = str_replace('[page]',$this->getIndex()-1,$attrs);
        $href = str_replace("\\", "",$baseUrl.($flag?$this->getIndex()-1:''));
		if($this->firstpage>1)$tem.="<li><a href='{$href}' {$attr}>上一页</a></li>";

		for($i=$this->firstpage;$i<=$this->lastpage;$i++)
		{
            $attr = str_replace('[page]',$i,$attrs);
            $href = str_replace("\\", "",$baseUrl.($flag?$i:''));
			if($i==$this->index)
			{
				$tem.="<li class='active'><a href='{$href}' {$attr}>{$i}</a></li>";
			}
			else
			{
        ...
        $attr = str_replace('[page]',$this->getIndex()+1,$attrs);
        $href = str_replace("\\", "",$baseUrl.($flag?$this->getIndex()+1:''));
		if($this->lastpage<$this->totalpage)$tem.="<li><a href='{$href}' {$attr}>下一页</a></li>";

		if($this->totalpage==0)$this->index=1;
		$attr = str_replace('[page]',$this->totalpage,$attrs);
		$href = $href = str_replace("\\", "",$baseUrl.($flag?$this->totalpage:''));
		return $tem."<li><a href='{$href}' {$attr}>尾页</a><span>当前第{$this->index}页/共{$this->totalpage}页</span></li></ul>";
	}
}
?>
