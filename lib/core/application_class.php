...
abstract class JApplication
{
    ...
    public function __construct($config)
    {
		...
		{
			JInterceptor::reg($config['interceptor']);
			register_shutdown_function(array('JInterceptor',"shutDown"));
		}
    }
    ...
    public function run()
    {
		JInterceptor::run("onCreateApp");
        $this->execRequest();
		JInterceptor::run("onFinishApp");
    }
    /**
     * @brief 实现应用的结束方法
    ...
    private function setDebugMode($flag)
    {
    	...
    	if($flag === true)
        {
			...
			JException::setDebugMode(true);
        }
		else
		{
			error_reporting(0);
			JException::setDebugMode(false);
		}
		set_error_handler("JException::phpError" , E_ALL|E_STRICT );
		set_exception_handler("JException::phpException");
		JException::setLogPath($basePath."backup/errorLog/".date("y-m-d").".log");
    }
	...
	public function getWebRunPath()
	{
		...
		{
			$this->webRunPath = JUrl::creatUrl('').str_replace(array(dirname(realpath(rtrim($_SERVER['DOCUMENT_ROOT'],'/\\')."/".ltrim($_SERVER['SCRIPT_NAME'],'/\\'))).DIRECTORY_SEPARATOR,'\\'),array('','/'),rtrim(realpath($this->getRuntimePath()),'/\\'));
		}
