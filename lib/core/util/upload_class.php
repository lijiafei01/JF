        ...
        foreach($_FILES as $field => $file)
        {
            ...
            if(is_array($_FILES[$field]['name']))
            {
                ...
                foreach($keys as $key)
                {
                	...
                	{
	                    //获取扩展名
	                    $fileext = JFile::getFileType($_FILES[$field]['tmp_name'][$key]);
	                    if(is_array($fileext) || $fileext == null)
	                    {
	                        $fileext = JFile::getFileSuffix($_FILES[$field]['name'][$key]);
	                    }

		                //图片木马检测
		                if(in_array($fileext,$this->checkType) && !JFilter::checkHex($_FILES[$field]['tmp_name'][$key]))
		                {
		                	$fileInfo[$key]['flag'] = -9;
		                }
		                else
		                {
		                    ...
		                    {
			                    //修改图片状态值
			                    $fileInfo[$key]['name']    = JTime::getDateTime('Ymdhis').mt_rand(100,999).'.'.$fileext;
			                    ...
			                    {
			                        if(JFile::xcopy($_FILES[$field]['tmp_name'][$key],$this->dir.$fileInfo[$key]['name']))
			                        {
			                            if(is_file($_FILES[$field]['tmp_name'][$key]))
			                            {
			                                JFile::unlink($_FILES[$field]['tmp_name'][$key]);
			                            }
			                        }
			                    }
		                    }
            ...
            else
            {
            	...
            	{
	                //获取扩展名
	                $fileext = JFile::getFileType($_FILES[$field]['tmp_name']);
	                if(is_array($fileext) || $fileext == null)
	                {
	                    $fileext = JFile::getFileSuffix($_FILES[$field]['name']);
	                }

                	//图片木马检测
                	if(in_array($fileext,$this->checkType) && !JFilter::checkHex($_FILES[$field]['tmp_name']))
                	{
                  	  	$fileInfo[0]['flag'] = -9;
                	}
                	else
                	{
		                ...
		                {
			                //修改图片状态值
			                $fileInfo[0]['name']    = JTime::getDateTime('Ymdhis').mt_rand(100,999).'.'.$fileext;
			                ...
			                {
			                    if(JFile::xcopy($_FILES[$field]['tmp_name'],$this->dir.$fileInfo[0]['name']))
			                    {
			                        if(is_file($_FILES[$field]['tmp_name']))
			                        {
			                            JFile::unlink($_FILES[$field]['tmp_name']);
			                        }
			                    }
			                }
		                }
