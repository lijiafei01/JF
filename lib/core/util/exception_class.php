...
class JException extends Exception
{
	...
	{
		if( !($e instanceof JException) && $e instanceof Exception  )
		{
			if( self::$debugMode )
			{
				echo $e->getMessage();
	...
	{
		$JF_path = JF_PATH;
		$iapp_path = JF::$app->getBasePath();

		$count = 1;

		if( strpos($path,$JF_path) ===0  )
		{
			$path = str_replace(JF_PATH , "JF_PATH/",$path , $count);
		}
	...
	public function show()
	{
		...
		if( ! $this instanceof JHttpException )
			self::logError($re);
...
class JHttpException extends JException
{
	/**
	 * @brief 获取控制器
	 * @return object 控制器对象
	...
	public function show()
	{
		...
		//检查用户是否定义了error处理类
		$config = isset( JF::$app->config['exception_handler'] ) ? JF::$app->config['exception_handler'] : 'Error' ;
		$flag = class_exists($config);
		if( $flag && method_exists($config,"error{$httpNum}") )
		{
			$errorObj = new $config(JF::$app,'error');
