...
class JUrl
{
	...
	public static function getInfo($key)
	{
		...
		return isset($arr[$key]) ? JReq::get($arr[$key]) : '';
	}

	/**
	 * @brief 将Url从JF支持的一种Url格式转成另一个格式。
	 * @param string $url 想转换的url
	 * @param int $from JUrl::UrlNative或者.....
	 * @param int $to JUrl::UrlPathinfo或者.....
	...
	private static function getRouteCache()
	{
		...
		//第一次初始化
		$routeList = isset(JF::$app->config['urlRoute']) ? JF::$app->config['urlRoute'] : array();
		if(empty($routeList))
		{
			self::$urlRoute = false;
			return null;
	...
	public static function creatUrl($url='')
	{
		...
		$rewriteRule = isset(JF::$app->config['rewriteRule'])?JF::$app->config['rewriteRule']:'native';
	...
	public static function getRefRoute()
	{
		...
		{
			$localUrlType = JF::$app->config['rewriteRule'] == 'pathinfo' ? self::UrlPathinfo : self::UrlNative;
	...
	public static function beginUrl($url='')
	{
		...
		//diy:/blog-100.html
		$obj = JServerVars::factory($_SERVER['SERVER_SOFTWARE']);
		...
		if( isset($urlArray[self::UrlCtrlName]) )
		{
			...
			if( count($tmp) == 2 )
			{
				JReq::set('module',$tmp[0]);
				JReq::set(self::UrlCtrlName , $tmp[1]);
			}
			else
			{
				JReq::set(self::UrlCtrlName , $urlArray[self::UrlCtrlName] );
			}
		}
		if( isset($urlArray[self::UrlActionName])  )
		{
			JReq::set(self::UrlActionName,$urlArray[self::UrlActionName]);
			if(JReq::get('action')=='run')
			{
				JReq::set('action',null);
			}
		}
...
class JServerVars implements IJServerVars
{
	public static function factory($server_type)
	{
		$obj = null;
		$type = array(
			'apache' => 'JServerVars_Apache',
			'iis'	=> 'JServerVars_IIS' ,
			'nginx' => 'JServerVars_Nginx'
		);
...
class JServerVars_Apache implements IJServerVars
{
...
class JServerVars_IIS implements IJServerVars
{
	public function __construct($server_type){}
...
class JServerVars_Nginx implements IJServerVars
{
	public function __construct($server_type){}
	public function requestUri()
	{
