...
class Config
{
    ...
    {
        if(isset(JF::$app->config['configExt'][$config]))
        {
        	$this->configFile = JF::$app->getBasePath().JF::$app->config['configExt'][$config];
	...
	public static function edit($configFile,$inputArray)
	{
		...
		foreach($inputArray as $key => $val)
		{
			...
			{
				$inputArray[$key] = JFilter::act($val,'text');
			}
		}
		...
		//写入配置文件
		$fileObj   = new JFile($configFile,'w+');
