...
class PhotoUpload
{
				...
				if($this->thumbWidth && $this->thumbHeight && $this->thumbKey)
				{
					foreach($this->thumbWidth as $thumbWidth_Key => $thumbWidth_Val)
					{
						...
						if(is_file($thumbFileName))
						{
							...
							if($this->tempup == 321) $result['thumb'][$this->thumbKey[$thumbWidth_Key]] = $photoRow['img'];
					...
					foreach($this->thumbKey as $thumbKey_key => $thumbKey_val)
					{
						...
						if($this->tempup == 321) $result['thumb'][$this->thumbKey[$thumbKey_key]] = $photoRow['img'];
					}
				}
				return $result;
			}
	 ...
	 */
	public static function thumb($fileName,$width,$height,$extName = '_thumb')
	{
	...
	{
		return IImage::img_water_mark($srcImg, $waterImg, $savepath, $savename, $positon, $alpha);
	}
				...
				if($val['flag']==1)
				{
					...
					if($this->thumbWidth && $this->thumbHeight && $this->thumbKey)
					{
						...
						foreach($this->thumbKey as $thumbKey_key => $thumbKey_val)
						{
							...
							if($this->tempup == 321){
								$rs[$innerKey]['thumb'][$this->thumbKey[$thumbKey_key]] = $fileName;
							}else {
