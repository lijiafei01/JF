...
class Mess
{
	...
	public static function sendToUser($update_where,$content)
	{
		...
		//插入$content
		$arr = array();
		$arr['title'] = JFilter::act($content['title']);
		$arr['content'] = JFilter::act($content['content'],'text');
		$arr['time'] = date('Y-m-d H:i:s');
		$tb = new JModel("message");
		$tb->setData($arr);
		$id = $tb->add();
		if($id === false)
		{
		...
		{
			$db = JDBFactory::getDB();
			$tableName = JF::$app->config['DB']['tablePre']."member";
